...
for i1=1:num_timeblocks
    ...
    for i2 = 1:num_noiseins
        ...
        for i3 = 1:num_runs
            ...
            data = generate_AGN_data(gt,T(i1),noise(i2));
            %spectral alg.
            g_init = algo_spectral_so2(data,E_n,T(i1),lam);
            corr_spectral(i1,i2,i3) = abs(dot(g_init,gt))/(n*T(i1));
            %ppm-DynSync alg. 
            g_ppm = algo_ppm_DynSync(data,E_n1,T(i1),g_init,lam,num_iter_ppm);
            corr_ppm(i1,i2,i3) = abs(dot(g_ppm,gt))/(n*T(i1));
            %GTRS-DynSync alg.(global TRS)
            g_trs = algo_GTRS_DynSync(data,E_n1,T(i1),lam);
            corr_trs_gl(i1,i2,i3) =  abs(dot(g_trs, gt))/(n*T(i1));
            %LTRS-GS-DynSync alg. (local TRS+low freq.proj.)
            g_trs_b = algo_LTRS_GS_DynSync(data,T(i1),P_tau_n1);
            corr_trs_b(i1,i2,i3) =  abs(dot(g_trs_b, gt))/(n*T(i1));
            %GMD-LTRS-DynSync alg. (global matrix denoising+ local sync.)
            g_denproj = algo_GMD_LTRS_DynSync(data,T(i1),P_tau_n);
