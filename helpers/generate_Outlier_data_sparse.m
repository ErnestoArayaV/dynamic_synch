...
function  [ data_mtx, G_union, list_Gk ] = generate_Outlier_data_sparse(gt, T, sigma, p)
n = size(gt,1)/T;
list_Gk = struct([]);
...
for k=1:T
    ...
    H_k_sparse_wOutliers = addOutlierNoise( H_k_sparse, G_k, sigma);
    H_k_sparse_wOutliers  = 1/2 * (H_k_sparse_wOutliers  + H_k_sparse_wOutliers');

    data_mtx((k-1)*n+1 : k*n,:) = H_k_sparse_wOutliers;
    
    isH = ishermitian(H_k_sparse_wOutliers);
    disp(['isHermitian=' int2str(isH)]);
end
