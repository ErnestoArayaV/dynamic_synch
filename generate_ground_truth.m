...
%              proj_mat - used to define a 'low frequency' space. See
%              projmat_smalleigs_lap.m
%Output:       gt       -'low frenquency' ground truth vector. Size n*Tx1

function gt = generate_ground_truth(n,T,proj_mat)
proj_small_eigs = kron(proj_mat,speye(n));
rand_angles = 2*pi*rand(n*T,1);
gt = exp(rand_angles*complex(0,1));
gt = proj_small_eigs*gt;
gt = proj_1(gt);% projection to achieve unit norm
