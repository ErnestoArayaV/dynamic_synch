...
MANY_STD = permute(MANY_STD,[1 2 3]);
...
for i = indexMethods
    ...
    if plot_error_bars == 0
        plot(ALL(:,1),ALL(:,1+i), 'LineStyle', linespec{i}, ...
            'Marker', markers{i}, 'LineWidth', 3, ...
            'MarkerSize',10,   'Color', Plot_color, ...
            "MarkerEdgeColor",Plot_color,"MarkerFaceColor",Plot_color); 
        hold on;
    elseif plot_error_bars ==1
        err = MTX_STD_ALL(:,1+i);
        scaling_coeff = 1
        % scaling_coeff = 0.25;
        err = scaling_coeff * err;  
        % errorbar(ALL(:,1),ALL(:,1+i), err, 'vertical', 'o', "MarkerEdgeColor", Plot_color);
        % "-s","MarkerSize",10, "MarkerEdgeColor","blue","MarkerFaceColor",[0.65 0.85 0.90]
        errorbar(ALL(:,1),ALL(:,1+i), err, 'vertical', ...
             'Marker', markers{i}, 'LineWidth', 3, ...
             "MarkerSize",10,  'Color',Plot_color,  ...
             "MarkerEdgeColor",Plot_color,  "MarkerFaceColor",Plot_color, ... 
             "CapSize",30);
...
if plot_error_bars == 111
    hold on;
    for i = indexMethods
        Plot_color = colorspec{i};
        err = MTX_STD_ALL(:,1+i);
        hold on;
        errorbar(ALL(:,1),ALL(:,1+i), err, 'both', 'o', "MarkerEdgeColor", Plot_color);  % "MarkerFaceColor", "black" , [0.65 0.85 0.90]
    end
end
